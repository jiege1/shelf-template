...
import { positiveOrZero, modifyObjValues } from './utils';
...
/**
 * 滚动区域组件
 * from https://github.com/souhe/reactScrollbar
 * API无修改
 * Method 改为通过 Provider 透传
 */
...
class ScrollArea extends React.Component {
  ...
  render() {
    ...
    let cls = classnames(css.scrollarea, className);
    let contentCls = classnames(css.scrollareaContent, contentClassName);
  ...
  handleKeyDown(e) {
    ...
    if (e.target.tagName.toLowerCase() !== 'input') {
      let deltaY = 0;
      let deltaX = 0;
      let _lineHeight = this.lineHeightPx ? this.lineHeightPx : 10;
